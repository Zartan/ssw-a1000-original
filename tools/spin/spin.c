...
#define NUM_IMAGES	16		/* number of images in 360 degrees */

#define MULT		7
#define OFFSET		(S_SIZE * MULT / 2)
#define X_MIN		(CENT_X - OFFSET)
#define X_MAX		(CENT_X + OFFSET - 1)
#define Y_MIN		(CENT_Y - OFFSET)
#define Y_MAX		(CENT_Y + OFFSET - 1)
#define CENT_X		(WIDTH / 2)
#define CENT_Y		(HEIGHT / 2)
...
struct NewScreen	NewScreen = {
	...
	NULL, NULL,			/* default font, screen title	*/
...
initialize()
{
	...
	if ((outf = fopen("RAM:image", "w")) == NULL)
		die(6);
	fprintf(outf, "\n");
}
...
scan_image()
{
	...
	for (row = Y_MIN; row <= Y_MAX; row += MULT) {
		commp = image;
		for (col = X_MIN; col <= X_MAX; col += MULT) {
...
draw_ring(int color, real_t radius, real_t width)
{
	...
	SetAPen(rp, color);
	r = MULT * radius;
	DrawCircle(rp, CENT_X, CENT_Y, (long)r);
	r += MULT * width;
	DrawCircle(rp, CENT_X, CENT_Y, (long)r);
	SetOPen(rp, color);
	r -= MULT/2 * width;
	Flood(rp, 0, CENT_X + (long)r, CENT_Y);
}
	  ...
	  register coord_t *cp, register int n)
{
	...
	cos_a *= MULT;
	for (flag = -1; --n >= 0; cp++) {
		x = CENT_X + (long)(cp->x * cos_a - cp->y * sin_a);
		if (x < 0L)
			x = 0L;
		else if (x >= WIDTH)
			x = WIDTH - 1L;
		y = CENT_Y + (long)(cp->x * sin_a + cp->y * cos_a);
		if (y < 0L)
			y = 0L;
		else if (y >= HEIGHT)
			y = HEIGHT - 1L;
